  ...
  return (
    <>
      ...
      <instancedMesh ref={mesh} args={[null, null, count]}>
        ...
        <meshPhongMaterial color={color} emissive={'#06b6d4'} emissiveIntensity={0.5} roughness={0} />
      </instancedMesh>
    </>
  )
}
...
export default function WebGLBackground({ darkMode }) {
  if (typeof window !== 'undefined' && window.matchMedia('(pointer: coarse)').matches) {
    return null; // Skip heavy 3D on mobile for performance
  }
  ...
  return (
    <div className="fixed inset-0 z-0 pointer-events-none opacity-40">
      <Canvas camera={{ position: [0, 0, 30], fov: 75 }}>
        ...
        <Particles count={600} color={darkMode ? '#a855f7' : '#db2777'} />
        {/* Secondary layer of cyan/pink particles */}
        <Particles count={300} color={darkMode ? '#06b6d4' : '#a855f7'} />
      </Canvas>
    </div>
  )
}
